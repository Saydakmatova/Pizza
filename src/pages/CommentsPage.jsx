        ...
        <ul className="posts" style={{ marginTop: 20 }}>
          {posts.map((item) => (
            <li key={item.id} style={{flexDirection: 'row', display: 'flex'}}>
              <img width={50} style={{borderRadius: 25, marginRight: 8}} src={item.user.photoURL}/>
              <div>
                {item.user.displayName} - {moment(item.created_at).calendar().toLowerCase()}<br/>{item.content}
              </div>
            </li>
          ))}
        </ul>
